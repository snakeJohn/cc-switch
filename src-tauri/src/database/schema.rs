...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    14 => {
                        log::info!("迁移数据库从 v14 到 v15（添加 Grok Skills 支持）");
    ...
    /// v14 -> v15：Skills 启用状态增加 Grok（`apps.grok` / `enabled_grok`）。
...
mod tests {
    ...
    #[test]
    fn migrate_v14_to_v15_adds_skills_enabled_grok_column() -> Result<(), AppError> {
        let conn = Connection::open_in_memory()?;
        conn.execute(
            "CREATE TABLE skills (
                id TEXT PRIMARY KEY,
